import React from 'react';
import SVG from '@splunk/react-icons/SVG';
export interface IconBaseProps extends React.ComponentProps<typeof SVG> {
    children?: React.ReactNode;
    /**
     *  Defaults to outlined, determines the variant of the icon to be rendered
     */
    variant?: 'default' | 'outlined' | 'filled';
}
/**
 * @description Triangular arrow pointing down, dropdown menu or downward navigation
 * @category Objects
 * @keywords arrow
 * @variants outlined,filled
 */
declare const ArrowTriangleDown: ({ children, variant, ...otherprops }: IconBaseProps) => React.JSX.Element | null;
export default ArrowTriangleDown;
