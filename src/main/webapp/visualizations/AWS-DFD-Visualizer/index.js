...
// Master Build: CRITICAL FIX FOR SOURCE LOADING + IMPROVED SPREAD
// =============================================================================
define([
    'splunkjs/mvc',
    'underscore',
    ...
    './d3.v7.min'
], function(mvc, _, SplunkVisualizationBase, visualizationUtils, d3) {
    ...
    var ICON_MAP = {
        ...
        'KINESISFIREHOSE': ARCH_SVC + 'Arch_Analytics/64/Arch_Amazon-Kinesis-Data-Firehose_64.svg',
        ...
        'CLOUDTRAIL': ARCH_SVC + 'Arch_Security-Identity/64/Arch_AWS-CloudTrail_64.svg',
        'TRAIL':      ARCH_SVC + 'Arch_Security-Identity/64/Arch_AWS-CloudTrail_64.svg',
    ...
    function getIconPath(type) {
        ...
        var service = (parts[parts.length - 1] || '').toUpperCase();
        var domain  = (parts[1] || '').toUpperCase();
        
        var iconPath = ICON_MAP[service] || ICON_MAP[domain] || 'generic.svg';
        var fullPath = ICON_BASE + iconPath;
        
        console.log('AWS-DFD Resolver Cache-Bust:', { type: type, service: service, domain: domain, fullPath: fullPath });
        return fullPath;
    }
    ...
    return SplunkVisualizationBase.extend({
        ...
        initialize: function() {
            ...
            this.$el = $(this.el); // jQuery is standard in Splunk 9.x global namespace
            this.$el.addClass('aws-composer-viz');
            this.$el.css({ 'background-color': '#F2F3F3', 'overflow': 'hidden' });
            console.log('AWS-DFD 1.9.2: Initialized');
        },

        updateView: function(data, config) {
            console.log('AWS-DFD 1.9.2: updateView');
            ...
            if (!d3Obj || typeof d3Obj.select !== 'function') {
                console.error('AWS-DFD 1.9.2: D3 NOT LOADED!', d3);
                return;
            }
            ...
            zones.append('rect')
                ...
                .attr('stroke', '#B1B1B1').attr('stroke-dasharray', '12,6').attr('rx', 25);
            zones.append('text')
                .attr('fill', '#838e9c').attr('font-size', '18px').attr('font-weight', 'bold');

            var link = linkLayer.selectAll('path').data(dataset.links).enter().append('path')
                .attr('stroke', '#879196').attr('fill', 'none').attr('stroke-width', 3);

            var linkLabels = labelLayer.selectAll('.link-label').data(dataset.links).enter().append('g');
            linkLabels.append('rect')
                .attr('width', 150).attr('height', 30).attr('rx', 15)
                .attr('fill', 'white').attr('stroke', '#D5D7D8').attr('x', -75).attr('y', -15);
            linkLabels.append('text')
                .attr('text-anchor', 'middle').attr('dy', 8)
                .attr('font-size', '14px').attr('fill', '#232F3E')
            ...
            node.append('rect')
                .attr('width', 280).attr('height', 100).attr('x', -140).attr('y', -50)
                .attr('fill', 'white').attr('stroke', '#D5D7D8').attr('rx', 12)
                .style('filter', 'drop-shadow(0px 8px 12px rgba(0,0,0,0.22))');

            node.append('rect')
                .attr('width', 66).attr('height', 66).attr('x', -128).attr('y', -33)
                .attr('fill', '#232F3E').attr('rx', 10);
            ...
            node.append('text')
                .attr('x', -45).attr('y', -14)
                .attr('font-size', '14px').attr('fill', '#545b64')
                .text(function(d) { return (d.type || 'AWS::Resource').replace('AWS::', ''); });

            node.append('text')
                .attr('x', -45).attr('y', 22)
                .attr('font-size', '18px').attr('font-weight', 'bold').attr('fill', '#232f3e')
                .text(function(d) { 
            ...
            } catch (err) { console.error('AWS-DFD Internal Error:', err); }
        },
