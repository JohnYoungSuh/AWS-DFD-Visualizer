...
const parseSplunkData = (data) => {
    ...
    rows.forEach(row => {
        ...
        if (isObjectMode) {
            ...
            rawLabel = row?.display_name || row?.node_label || row?.label;
            ...
            const iType = indexOfFirstField(fields, ['resource_type', 'type', 'component_type']);
            rawType  = (row && iType > -1) ? row[iType] : (row ? (row[2] || 'AWS::Resource') : 'AWS::Resource');
            const iLabel = indexOfFirstField(fields, ['display_name', 'node_label', 'label']);
